...
The code below is self-explanatory. We are using the `qplot` function (which stands for __q__uickplot), and asking it to draw us a `line` plot of `year` vs. `prop`, both of which are to be looked up in the dataset `dirk`.
...
```{r echo = F}
# result <- ddply(bnames2_b, "name", summarize, name = name[1], tot = sum(prop*births))
```
